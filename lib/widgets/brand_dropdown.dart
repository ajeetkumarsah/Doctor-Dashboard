...
class SingleBrandSelect extends StatefulWidget {
  final List<BrandModel> items;
  final String label;
  final bool invert;
  final BrandModel? selectedValue;
  final void Function(BrandModel?)? onChanged;
  const SingleBrandSelect({
    super.key,
    required this.items,
    required this.label,
    this.invert = false,
  ...
  @override
  State<SingleBrandSelect> createState() => _SingleSelectState();
}

class _SingleSelectState extends State<SingleBrandSelect> {
  @override
  Widget build(BuildContext context) {
    return DropdownButtonHideUnderline(
      child: DropdownButton2<BrandModel>(
        ...
        items: widget.items
            .map((BrandModel item) => DropdownMenuItem<BrandModel>(
                  value: item,
                  child: Column(
